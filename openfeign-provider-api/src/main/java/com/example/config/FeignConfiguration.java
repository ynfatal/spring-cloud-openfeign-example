 ...
 * 理解：
 * Spring 容器中，相同类型相同名字的Bean是不可能存在的，而在子上下文之间，可以这样配置，但是只会实例化第一个，也就是全局的那个。debug 一下你就知道了。
 * 扩展：
 * 其实这里的自定义配置没必要标注 @Configuration，因为这里的配置是给子上下文用的，标注后，在父上下文中又没用。
 * 如果在父上下文中需要用到，可以添加标注，否则，去掉吧。
 * ===========  方式二  ===========
 * 配置行为如下：
 * - 自定义全局：不使用 org.springframework.cloud.openfeign.EnableFeignClients#defaultConfiguration() 指定自定义全局配置类，该配置类需要标注为配置组件
 * 放在 ComponentScan 能扫描的地方即可
 * - 自定义局部：同方式一
 * 这种方式不需要考虑方式一的注意事项，但是建议考虑比较好。
 * 理解：建议使用第一种方式，不标注 @Configuration，要不然在父上下文和子上下文中都有这套 Bean，但是父上下文的又没有被用到，实属多余。
 * @author Fatal
 * @date 2020/6/28 8:47
 */
